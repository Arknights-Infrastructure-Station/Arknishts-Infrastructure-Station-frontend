<template>
  <div class="form-container">
    ...
    <div class="box">
      <el-form :model="forgetPasswordData">
        <h2>忘记密码</h2>
        <div class="input-box">
          <input v-model="forgetPasswordData.email" type="text" @keydown.enter="handleEnter('email')">
          <span v-show="forgetPasswordData.email===''" class="label-text">邮箱</span>
          <span v-show="forgetPasswordData.email!==''" class="exist-label-text">邮箱</span>
          <i class="input-background"/><i class="input-background-cover"/>
        </div>
        <div class="input-box">
          <input v-model="forgetPasswordData.password" type="password" @keydown.enter="handleEnter('password')">
          <span v-show="forgetPasswordData.password===''" class="label-text">新密码</span>
          <span v-show="forgetPasswordData.password!==''" class="exist-label-text">新密码</span>
          <i class="input-background"/><i class="input-background-cover"/>
        </div>
        <div class="input-box">
          <input ref="confirmPasswordInput" v-model="forgetPasswordData.confirmPassword" type="password" @keydown.enter="handleEnter('confirmPassword')">
          <span v-show="forgetPasswordData.confirmPassword===''" class="label-text">确认新密码</span>
          <span v-show="forgetPasswordData.confirmPassword!==''" class="exist-label-text">确认新密码</span>
          <span class="confirmPassword-tip"/>
          <i class="input-background"/><i class="input-background-cover"/>
        </div>
        <div class="input-box">
          <input class="verification-code" v-model="forgetPasswordData.verificationCode"
          ...
          <button
              ...
              @click="tryToSendCode(forgetPasswordData.email)"
          >{{ buttonLabel }}
          </button>
          <span v-show="forgetPasswordData.verificationCode===''" class="label-text">验证码</span>
          <span v-show="forgetPasswordData.verificationCode!==''" class="exist-label-text">验证码</span>
        ...
        <div class="button-text">重设密码</div>
      </el-form>
    </div>
  </div>
</template>
...
import {buttonLabel, handleSendCode, updateCooldown, initSendTimes} from "@/utils/sendCodeCooldown.js";
...
const forgetPasswordData = reactive({
  email: '',
  password: '',
  verificationCode: '',
  confirmPassword: '',
...
watchEffect(() => {
  forgetPasswordData.isAllRight = forgetPasswordData.email !== '' &&
      forgetPasswordData.password !== '' &&
      forgetPasswordData.confirmPassword !== '' &&
      forgetPasswordData.verificationCode !== '';
});

//提交表单
async function submitForm() {
  if (!forgetPasswordData.isAllRight) {
...
function handleEnter(field) {
  if (forgetPasswordData.isAllRight) {
    ...
    if (field === 'email' && !forgetPasswordData.email) {
      focusNextInput('email');
    } else if (field === 'password' && !forgetPasswordData.password) {
      focusNextInput('password');
    } else if (field === 'confirmPassword' && !forgetPasswordData.confirmPassword) {
      focusNextInput('confirmPassword');
    } else if (field === 'verificationCode' && !forgetPasswordData.verificationCode) {
...
onMounted(() => {
  ...
  document.querySelectorAll('.box .el-form .input-box').forEach(inputBox => {
    ...
    confirmPasswordInput.value.addEventListener('focus', () => {
      confirmPasswordWatch = watch(() => forgetPasswordData.confirmPassword, (newVal) => {
        confirmPasswordTip.textContent = newVal !== forgetPasswordData.password ? '密码不一致' : '';
        // console.log('检测成功')
      });
    });
...
.box .el-form .input-box .confirmPassword-tip {
  position: absolute;
  width: 100%;
  top: 52px;
  right: -240px;
