...
import {getPngByKey, getStarListForUser, starWorkFile, unstarWorkFile} from "@/api/callEndpoint.js";
...
// 作业下载
const downloadWorkFile = async () => {
  let formattedWorkFileContent;
  let blob;
  let fileExtension = 'json';

  // 如果作业类型为 Mower，尝试重构作业文件的基建布局部分
  if (workFileDetail.type === 'Mower') {
    // Mower 作业分为 text 存储和 pictureKey 存储
    if (workFileDetail.storageType === 'pictureKey') {
      // 处理 pictureKey 存储类型的作业
      try {
    ...
    formattedWorkFileContent = JSON.stringify(JSON.parse(workFileDetail.fileContent)); // 将作业文件重新格式化（进行压缩）
    blob = new Blob([formattedWorkFileContent], {type: "application/json"});
  }
